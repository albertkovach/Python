...
class GUI(Frame):
    ...
    def initUI(self):
        ...
        OutputNameLbl = Label(text="Имя файла PDF:", background="white")
...
def RegistryFillingThread():
    ...
    OutputName = OutputNameEntry.get() + ".pdf"
    print(OutputName)
    OutputFile = Path(desktop, OutputName)
    ...
    movedir = Path(desktop, OutputNameEntry.get())
    ismovedirexist = os.path.exists(movedir)
    if not ismovedirexist:
        os.makedirs(movedir)
        

    
    for i in range(0, len(InputFilesArray)):
        filename = Path(InputFilesArray[i])
        ofilename = str(i)+".pdf"
        
        MoveFilesPath = Path(movedir, ofilename)
        RevisedMoveFilesPath = MoveFilesPath.as_posix()
        shutil.copyfile(InputFilesArray[i], RevisedMoveFilesPath)
        tempfilesarray.append(RevisedMoveFilesPath)


    for f in range(len(tempfilesarray)):
        
        fileprecent = int(filenum / len(tempfilesarray) * 100)
        filenum = filenum + 1
        ProgressLbl.config(text = 'Обработка: {0} из {1}. Выполнено: {2}%'.format(filenum, len(tempfilesarray), fileprecent)) 
        print('Документ №{0}: {1}'.format(f+1, Path(tempfilesarray[f]).name))
        
        pdfmerger.append(tempfilesarray[f])
        pdfmerger.append(Path(desktop, 'blank.pdf').as_posix())
        
    ProgressLbl.config(text = 'Запись файла...')
    pdfmerger.write(OutputFile.as_posix())
...
def PDFadd2endpages(in_fpath, out_fpath):
