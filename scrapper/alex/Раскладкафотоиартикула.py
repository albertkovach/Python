...
for i in range(3,sheet.max_row+1):
    try:
        photos = str(sheet.cell(i,6).value).split(";")
    except:
        print("FAILURE")
    sheet.cell(i,3).value = photos[0]
    print(len(photos))
    photos2 = ""
    if len(photos)>1:
        photos2 = str(photos[1])
    if len(photos)>2:
        for photo in range(2,len(photos)):
            photos2=photos2+";"+str(photos[photo-1])
    sheet.cell(i,4).value = photos2
    #sheet.cell(i,2).value = (sheet.cell(i,7).value).split(" ")[0]
    print(photos)
input("Press Enter to continue...")
try:
    book.save(InputFile)
