...
class GUI(Frame):
    ...
    def initUI(self):
        ...
        URLText.insert(END,"https://www.sds-group.ru/catalog_2341.htm")
...
def p3():
    ...
    counter1 = 0
    while(nextpaging):
        links = soup.find_all('a', {'class' : 'img'})
        ...
        for link in links:
            ...
            print(link['href'])
            urls.append('https://www.sds-group.ru'+link['href'])
        #print(url)
        #print(counter1)
        nextpageli = soup.find('li',{'class':'last-arrow'})
        try:
        ...
        if nextpage and counter1<200:
            print(nextpage['href'])
            page = requests.get('https://www.sds-group.ru'+nextpage['href'])
    ...
    for url in urls:
       ...
       #     n=data.text
        ...
        try:
            ...
            descrч = soup.find('div',{'class' : 'box-catalog_detail-top'})
            descr = descrч.find('meta',{'itemprop' : 'description'})
            #print(nomenkls)
            #for n in nomenkls:
                #nomenkl = n.find('span')
                #expsheet.cell(counter,3+nomenkls.index(n)).value = descr.text
                #print()
            expsheet.cell(counter,1).value = descr['content']
        except:
            print("FAILURE")
        
        try:
            article = soup.find('meta',{'itemprop' : 'sku'})
            #print(nomenkls)
            #for n in nomenkls:
                #nomenkl = n.find('span')
                #expsheet.cell(counter,3+nomenkls.index(n)).value = descr.text
                #print()
            expsheet.cell(counter,3).value = article['content']
        except:
            print("FAILURE")    
        ...
        counterX = 2
        try:
            imagesx = soup.find('div', {'class' : 'swiper-wrapper'})
            ...
            for image in images:
                ...
                if not image in control:
                    ...
                    expsheet.cell(counter,counterX).value = str(expsheet.cell(counter,counterX).value)+";https://www.sds-group.ru"+image['href']
                #counterX=counterX+1
                #print(img['src'])
        except:
            print('fail')
