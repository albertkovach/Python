...
class GUI(Frame):
    ...
    def initUI(self):
        ...
        URLText.insert(END,"https://tdme.ru/category/Komplektuyuschie_dlya_svetilynikov")
...
def p3():
    ...
    while(nextpaging):
        ...
        links = grid.find_all('div', {'class' : 'w-full'})
        #print(links)
        ...
        for link in links:
            lnk = link.find('a')
            urls.append("https://tdme.ru/"+lnk['href'])
            print("https://tdme.ru/"+lnk['href'])
        #print(url)
        #next = soup.find('a',{'class':"pagination__link pagination__link--next"})
        
        #if next and counter1<100:
        #print(next['href'])
        #page = requests.get("https://tdme.ru/"+next['href'])
        #counter1=counter1+1
        #soup = BeautifulSoup(page.content, 'html.parser')
        #else:           
        nextpaging = False
    ...
    for url in urls:
       ...
       #     n=data.text
        ...
        print(url)
        try:
            nomenkl = soup.find_all('meta')
            print(nomenkl)
            #print(nomenkl[1])
            #print(nomenkl[1]['content'])
            expsheet.cell(counter,3).value = nomenkl[1]['content']
        except:
            print("FAILURE1")
        try:
        ...
        try:
            ...
            print("FAILURE2")
        #try:
            #video = soup.find('div',{'class' : 'tab-content tab-space'})
        ...
        counterX = 2
        images = soup.find_all('img', {'alt' : 'product-img'})
        for image in images:
            img = image.find('img')
            expsheet.cell(counter,counterX).value = str(expsheet.cell(counter,counterX).value)+"; https://tdme.ru/"+img['src']
            #counterX=counterX+1
            #print(img['src'])
  ...
  #                    E14
   ...
   #                   EF000048
     #               </span></td></tr>
        ...
        characteristics = soup.find_all('tr', {'class' : 'border-b odd:bg-white even:bg-gray-200'})
        for characteristic in characteristics:
            name = characteristic.find('td', {'class' : 'lg:w-1/3 lg:w-auto py-4 lg:px-6 text-xs break-all lg:break-normal lg:text-sm text-gray-700'})
            #values = characteristic.find_all('td', {'class' : 'card-tabs-table__cell'})
            value = characteristic.find('span')
            #valuea = value.find('a')
