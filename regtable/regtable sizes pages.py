...
def RegistryFillingThread():
    ...
    try:
        with open(CSVfile, 'w', newline='') as f:
            ...
            writer.writerow(["Папка", "Страниц", "Размер, МБ"])
    except:
    ...
    for f in range(len(InputFilesArray)):
        ...
        with pike.open(InputFilesArray[f]) as pdfdoc:
            pagecount = len(pdfdoc.pages)
            
            
            
        CSVrows.append([Path(InputFilesArray[f]).parent.name, pagecount, filesize])
